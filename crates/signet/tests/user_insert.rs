...
async fn every_optional_field_survives_the_round_trip() {
    ...
    // `users_username_key` and `users_phone_key` are unique, so these must be
    // unique per run. Fixed values here made the test depend on global database
    // state: one leaked row was enough to fail every later run.
    let username = format!("insert-{}", &tag[..8]);
    let phone = format!("+852{:08}", Uuid::new_v4().as_u128() % 100_000_000);
    ...
    // `USER_COLS` for SCIM to see the id it provisioned, so it is the one most
    // likely to go missing from one of the two lists.
    let read = user_by_id(&state.pool, id).await.expect("read it back");
...
async fn status_is_settable_because_scim_provisions_disabled_accounts() {
    ...
    // `active_user_by_id` must refuse it while `user_by_id` still returns it:
    // SCIM can create a disabled account, and nothing may mint a credential
    // for one.
...
async fn a_unique_violation_is_returned_raw_for_the_caller_to_describe() {
    ...
    common::delete_user(&state.pool, first).await;
}
